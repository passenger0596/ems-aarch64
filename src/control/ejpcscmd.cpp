...
// 设置有功功率
void EjPcsCmd::pcs_set_power(const double& power,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
    uint16_t raw_value = static_cast<uint16_t>(static_cast<int>(power*10));  // 将有功功率乘以10并转换为无符号整数
    std::shared_ptr<ModbusClient>& mb_client = this->modbus_clients_[com];
...
// 设置直流充电电压
void EjPcsCmd::pcs_set_dcChargeVol(const double& dc_charge_vol,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
// 设置无功功率
void EjPcsCmd::pcs_set_reactivePower(const double& reactive_power,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
    uint16_t raw_value = static_cast<uint16_t>(static_cast<int>(reactive_power*10));  // 将无功功率乘以10并转换为无符号整数
    std::shared_ptr<ModbusClient>& mb_client = this->modbus_clients_[com];
...
// 设置频率
void EjPcsCmd::pcs_set_frequency(const double& frequency,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
// 设置直流母线电压
void EjPcsCmd::pcs_set_dcBusVol(const double& dc_bus_vol,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
// 设置母线过压保护点
void EjPcsCmd::pcs_set_busHighProtect(const double& bus_high_protect,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
// 设置母线欠压保护点
void EjPcsCmd::pcs_set_busLowProtect(const double& bus_low_protect,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
// 设置交流过压保护点
void EjPcsCmd::pcs_set_acHighProtect(const double& ac_high_protect,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
// 设置交流欠压保护点
void EjPcsCmd::pcs_set_acLowProtect(const double& ac_low_protect,const std::string& mode,std::shared_ptr<Device>& device) {
    int com = device->get_com();
...
void EjPcsCmd::process_pcs_commands(const std::string& device_name) {
    try {
        ...
        // 处理固化参数命令
        if (device_commands.contains("setSolidParam") && device_commands["setSolidParam"].is_number() && device_commands["setSolidParam"].get<int>()) {
            LOG_INFO_LOC("准备执行" + device_name + " 固化参数命令");
            pcs_solid_param("manual", device);
            device_commands["setSolidParam"] = false;
        }
        ...
        //     int16_t charge_vol = device_commands["setDcBusVol"].get<int16_t>();
        ...
        if (device_commands.contains("setReactivePower") && !device_commands["setReactivePower"].is_null()) {
            double q_power = device_commands["setReactivePower"].get<double>();
        ...
        // 处理直流母线电压设置
        if (device_commands.contains("setDcBusVol") && !device_commands["setDcBusVol"].is_null()) {
            double dc_bus_vol = device_commands["setDcBusVol"].get<double>();
        ...
        if (device_commands.contains("setBusHighProtect") && !device_commands["setBusHighProtect"].is_null()) {
            double bus_high_protect = device_commands["setBusHighProtect"].get<double>();
        ...
        if (device_commands.contains("setBusLowProtect") && !device_commands["setBusLowProtect"].is_null()) {
            double bus_low_protect = device_commands["setBusLowProtect"].get<double>();
        ...
        if (device_commands.contains("setAcHighProtect") && !device_commands["setAcHighProtect"].is_null()) {
            double ac_high_protect = device_commands["setAcHighProtect"].get<double>();
        ...
        if (device_commands.contains("setAcLowProtect") && !device_commands["setAcLowProtect"].is_null()) {
            double ac_low_protect = device_commands["setAcLowProtect"].get<double>();
