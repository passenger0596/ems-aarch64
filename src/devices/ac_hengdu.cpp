...
AcHengdu::AcHengdu(const std::string& name, int com, int id)
    : Device(name, com, id) {
    this->name_ = name;
    this->id_ = id;
    this->com_ = com;
    ...
    // 加载配置，使用ACHengdu专用的配置文件路径
    init_config(Config::AC_HENGDU_COMMUNICATION_FILEPATH);
    ...
    this->segments02_ = Device::generate_segments_from_addresses(all_addresses, 2000);
    ...
    this->segments04_ = Device::generate_segments_from_addresses(all_addresses, 124);
...
void AcHengdu::init_config(const std::string& config_file) {
...
void AcHengdu::read_data(ModbusClient& mb_client)
{
